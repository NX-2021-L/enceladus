/**
 * documentSections.ts — client-side section slicing for the outline-first
 * document view (ENC-TSK-P92, ports ENC-TSK-P80 FR-B5-1..3, B5-5..6).
 *
 * The document API contract gives us the full body (`Document.content`) and
 * an outline of {heading_path, level, ordinal, block_id, line_start,
 * line_end} per section, but explicitly documents line_start/line_end as
...
/** Small local slug helper — no shared slugify utility exists in ui-v2 yet,
 *  and pulling one in is out of scope for this port. */
 ...
 * sections whose content changed are marked "changed"). Removed sections
 * are not reported — there is nothing left to mark.
 */
