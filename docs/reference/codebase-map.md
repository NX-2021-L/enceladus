...
| **Warehouse (T2) registration** | The single sanctioned way to put a table into the governed warehouse: Parquet write plus **declared** Glue registration in one idempotent operation, full-refresh overwrite, mandatory freshness stamp. Replaces crawler-based schema discovery, which is a contract violation. | `backend/lambda/shared_layer/python/enceladus_shared/warehouse_registration.py`; `tools/warehouse_export_finance.py` (first caller + conformance check); `backend/lambda/shared_layer/test_warehouse_registration.py`; `docs/reference/warehouse-registration-contract.md` | `docs/reference/warehouse-registration-contract.md` |
| **Ad-hoc promotion (quarantine gate)** | The one way a table leaves the `hive.adhoc` quarantine namespace and enters a governed schema: io reviews a **pre-populated** type map, promotion applies the declared SQL types all-or-nothing, and the result is written through the warehouse registration library. Supplies `DATE`, `TIMESTAMP` and `DECIMAL` — the three types Superset's upload dialog structurally cannot express. Refuses any invocation without a human actor (decision D-4). **Not in this repo:** DVP-TSK-703 relocated it to `NX-2021-L/devops` because it operates on the Trino catalog and the warehouse, not on the governance ontology. It still writes through `enceladus_shared.warehouse_registration` below, fetched from here at a pinned commit rather than copied. | `NX-2021-L/devops` `services/lambda/adhoc_promotion/` | `NX-2021-L/devops` `services/lambda/adhoc_promotion/promotion_run.py` |
