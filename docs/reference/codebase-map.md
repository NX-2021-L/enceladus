...
| **Ad-hoc promotion (quarantine gate)** | The one way a table leaves the `hive.adhoc` quarantine namespace and enters a governed schema: io reviews a **pre-populated** type map, promotion applies the declared SQL types all-or-nothing, and the result is written through the warehouse registration library. Supplies `DATE`, `TIMESTAMP` and `DECIMAL` — the three types Superset's upload dialog structurally cannot express. Refuses any invocation without a human actor (decision D-4). | `backend/lambda/adhoc_promotion/promotion_plan.py` (type-map pre-population, risk R-6), `…/promotion_coerce.py` (all-or-nothing + offending-row report), `…/promotion_run.py` (writes via the shared library), `…/promotion_source.py`, `…/lambda_function.py`; `backend/lambda/adhoc_promotion/test_adhoc_promotion.py` | `backend/lambda/adhoc_promotion/promotion_run.py` |
