#!/usr/bin/env bash
# ENC-TSK-H47 — tombstone. scoring_service is built by the Gen2 matrix build (_build.yml,
...
echo "scoring_service is managed by the Gen2 pipeline (_build.yml + _deploy.yml); this is a no-op tombstone."
exit 0
