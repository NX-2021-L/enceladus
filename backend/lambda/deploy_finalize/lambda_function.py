...
# Prefix-to-project cache (ENC-TSK-O47: MINT `prefix` and `alias_prefixes`
# both resolve; a prefix cannot map to more than one project_id, so a single
# str->str cache still works, as long as MINT is loaded last/wins on
# conflict -- see _load_prefix_map).
...
# instead of silently resolving (mirrors tools/enceladus-mcp-server/
# server.py::_PREFIX_COLLISIONS and coordination_api/project_utils.py's
...
def _infer_record_type(record_id: str) -> Optional[str]:
    ...
    m = re.match(r"^[A-Z]{3}-(TSK|FTR|ISS|LSN)-[A-Z0-9]{3}(?:-[0-9][A-Z])?", record_id)
    if not m:
        return None
