#!/usr/bin/env bash
# ENC-TSK-H46 — tombstone. lifecycle_service is built by the Gen2 matrix build (_build.yml,
...
echo "lifecycle_service is managed by the Gen2 pipeline (_build.yml + _deploy.yml); this is a no-op tombstone."
exit 0
