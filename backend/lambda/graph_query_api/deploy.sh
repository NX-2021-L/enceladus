...
ensure_role() {
  ...
  local policy_file
  policy_file="$(mktemp /tmp/graph-query-api-policy-XXXXXX.json)"
...
POLICY
  aws iam put-role-policy \
    --role-name "${ROLE_NAME}" \
    --policy-name "devops-graph-query-api-inline" \
...
ensure_lambda() {
  local zip_path role_arn effective_key env_file
...
ensure_api_route() {
  ...
  local routes=(
    "GET /api/v1/tracker/graphsearch"
    "GET /api/v1/tracker/graphsearch/health"
    "OPTIONS /api/v1/tracker/graphsearch"
  )
  ...
  local stmt_id="allow-apigw-${API_ID}-graphsearch"
  if aws lambda add-permission \
    ...
    --source-arn "arn:aws:execute-api:${REGION}:${ACCOUNT_ID}:${API_ID}/*/*/api/v1/tracker/graphsearch*" >/dev/null 2>&1; then
