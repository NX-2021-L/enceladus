...
ENABLE_TYPED_RELATIONSHIPS="${ENABLE_TYPED_RELATIONSHIPS:-true}"
ENABLE_CONTEXT_NODES="${ENABLE_CONTEXT_NODES:-true}"
ENABLE_LESSON_PRIMITIVE="${ENABLE_LESSON_PRIMITIVE:-true}"
...
COMPONENT_EVENTS_TOPIC_ARN="${COMPONENT_EVENTS_TOPIC_ARN:-arn:aws:sns:${REGION}:${ACCOUNT_ID}:enceladus-component-registry-events${ENVIRONMENT_SUFFIX}}"
  ...
  cat > "${policy_file}" <<POLICY
{
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Effect": "Allow",
      "Action": ["sns:Publish"],
      "Resource": "${COMPONENT_EVENTS_TOPIC_ARN}"
    },
    {
...
ensure_lambda() {
  ...
  DEAD_LETTER_SNS_TOPIC_ARN="${DEAD_LETTER_SNS_TOPIC_ARN}" \
  COMPONENT_EVENTS_TOPIC_ARN="${COMPONENT_EVENTS_TOPIC_ARN}" \
...
if not isinstance(existing_env, dict):
    existing_env = {}
# ENC-ISS-107: purge stale keys that pushed env vars over 4KB.
# HOST_V2_* now use config.py defaults; ENCELADUS_COORDINATION_* are redundant
# copies of COORDINATION_INTERNAL_API_KEY*.
_stale = [k for k in existing_env if k.startswith("HOST_V2_") or k.startswith("ENCELADUS_COORDINATION")]
for k in _stale:
    del existing_env[k]
...
env_vars = {
    ...
    "ENABLE_TYPED_RELATIONSHIPS": os.environ.get("ENABLE_TYPED_RELATIONSHIPS", "true"),
    "ENABLE_CONTEXT_NODES": os.environ.get("ENABLE_CONTEXT_NODES", "true"),
    "ENABLE_LESSON_PRIMITIVE": os.environ.get("ENABLE_LESSON_PRIMITIVE", "true"),
